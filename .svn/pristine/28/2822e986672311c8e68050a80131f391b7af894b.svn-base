...
=begin rdoc
关键字名: 关闭所有NewClient
描述: 就是关闭所有的newClient程序窗口
维护人: 王沃伦
参数:
id=>hope,name=>期望结果,type=>s,must=>false,default=>"成功",value=>"成功|失败",descrip=>"期望结果"
=end
      def newClient_udp_close(hash={})
        AF::AutoIt.close_window_by_pid_name(nil,"TCP/UDP Testtool") do |closed_array|
          return_ok if closed_array.length > 0
          return_fail
        end
      end
