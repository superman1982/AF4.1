...
维护人: 王沃伦
参数:
id=>type,name=>协议类型,type=>s,must=>false,default=>"全部",value=>"全部|TCP|UDP|ICMP|其他",descrip=>"协议的类型,全部,TCP,UDP,ICMP,其他"
id=>proto_num,name=>协议号,type=>s,must=>false,default=>"0",value=>"{text}",descrip=>"具体的IP协议号,当协议类型为其他时有效"
id=>lan_ip,name=>内网IP选择,type=>s,must=>false,default=>"所有IP",value=>"所有IP|指定IP范围",descrip=>"适用的内网IP类型"
id=>lan_ip_range,name=>内网IP范围,type=>s,must=>false,default=>"2.2.2.2",value=>"{text}",descrip=>"内网IP范围时的范围"
id=>lan_port,name=>内网端口选择,type=>s,must=>false,default=>"所有端口",value=>"所有端口|指定端口范围",descrip=>"内网端口的类型"
id=>lan_port_range,name=>内网端口范围,type=>s,must=>false,default=>"111",value=>"{text}",descrip=>"内网端口的范围"
id=>wan_ip,name=>外网IP选择,type=>s,must=>false,default=>"所有IP",value=>"所有IP|指定IP范围",descrip=>"外网IP的类型"
id=>wan_ip_range,name=>外网IP范围,type=>s,must=>false,default=>"1.1.1.1",value=>"{text}",descrip=>"外网IP的范围"
id=>wan_port,name=>外网端口选择,type=>s,must=>false,default=>"所有端口",value=>"所有端口|指定端口范围",descrip=>"外网端口的类型"
id=>wan_port_range,name=>外网端口范围,type=>s,must=>false,default=>"100",value=>"{text}",descrip=>"外网端口的范围"
id=>target_line,name=>目标线路,type=>s,must=>false,default=>"1",value=>"1|2|3|4|5|6|7|8",descrip=>"目标线路号"
id=>hope,name=>期望结果,type=>s,must=>false,default=>"成功",value=>"成功|失败|none",descrip=>"期望结果为'none'时,将略过期望结果检查,直接认为执行成功,请慎重使用"
=end
